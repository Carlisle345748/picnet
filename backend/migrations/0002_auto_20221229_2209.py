...
def populate(apps, schema_editor):
    ...
    def create_user(username, first_name, last_name, description, password="123"):
        ...
        Profile(user=user, description=description, avatar=fake_avatar()).save()
        return user
    ...
    def create_photo(file_name, date_time, user, comments):
        img = open(f'media/{file_name}', 'rb')
        thumb_io = BytesIO()
        thumb_io.write(img.read())
        p = Photo(date_time=date_time, user=user)
        p.file_name.save(file_name, ContentFile(thumb_io.getvalue()))
        p.save()
        for c in comments:
            c.photo = p
            c.save()
